...
func (h *FCMHandler) ShowNotifyHistory(w http.ResponseWriter, r *http.Request) {
	var newNotify models.NotifyHistory
...
func (h *FCMHandler) GetNotifyHistory(userId int) ([]models.NotifyHistory, error) {
	if h.DB == nil {
		log.Print("h.DB is nil")
		return []models.NotifyHistory{}, fmt.Errorf("database connection is not initialized")
	}

	var notifications []models.NotifyHistory

	query := "SELECT * FROM notify_history WHERE user_id = ?"
	rows, err := h.DB.Query(query, userId)
	if err != nil {
		return []models.NotifyHistory{}, err
	}
	defer rows.Close()

	for rows.Next() {
		var notification models.NotifyHistory
